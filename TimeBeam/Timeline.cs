...
namespace TimeBeam
{
    //TODO:  Make the graph fit better on start up
    //TODO:  Shortcut key to resize graph, if all current trace transitions are out of view we want to fix that
    //TODO:  Create pop up information box about Trackitem when selected?
    //TODO:  Enable user to hide traces they don't care about.
    //TODO:  Have the clock show tick marks and current units as well as the time currently placed at
    //TODO:  Play button that moves the playhead at a slow speed activating popups
    /// <summary>
    ...
    public partial class Timeline : Control
    {
        ...
        ///   The tracks currently placed on the timeline.
        /// </summary>
        public List<IMultiPartTimelineTrack> Tracks { get { return _tracks; } }
        ...
        {
            if (!(((_tracks.Count * (TrackHeight + TrackSpacing)) * _renderingScale.Y) >= int.MaxValue))
        ...
        protected override void OnKeyUp(KeyEventArgs e)
        {
            ...
            {
                if (e.KeyCode == (Keys.LButton | Keys.MButton | Keys.Back | Keys.ShiftKey | Keys.Space | Keys.F17)
                    || e.KeyCode == Keys.Subtract)
                {
                    float amount = -120 / 1200f;
                    Rectangle trackAreaBounds = GetTrackAreaBounds();
                    if (IsKeyDown(Keys.Alt))
                    {
                ...
                else if (e.KeyCode == (Keys.LButton | Keys.RButton | Keys.Back | Keys.ShiftKey | Keys.Space | Keys.F17)
                    || e.KeyCode == Keys.Add)
                {
                    float amount = 120 / 1200f;
                    Rectangle trackAreaBounds = GetTrackAreaBounds();
